...
function BalanceSheet({ balanceSheet, historicalData = [] }) {
  ...
  // Updated categorization: Residential construction (F158) now in Consumer, commercial construction (F159) in Business
  const calculateLoanCategories = () => {
    ...
    const residentialConstruction = portfolio.realEstate.constructionAndLandDevelopment?.residential1To4Family || 0;
    ...
    const consumerTotal = residential + residentialConstruction + creditCards + auto + otherConsumer + consumerLeases;

    // Business Lending
    const commercialConstruction = portfolio.realEstate.constructionAndLandDevelopment?.otherConstructionAndLandDevelopment || 0;
    const multifamily = portfolio.realEstate.multifamily || 0;
    const creOwnerOccupied = portfolio.realEstate.nonfarmNonresidential?.ownerOccupied || 0;
    const creOther = portfolio.realEstate.nonfarmNonresidential?.otherNonfarmNonresidential || 0;
    const commercialRE = commercialConstruction + multifamily + creOwnerOccupied + creOther;
    ...
    const agricultural = (portfolio.other.agriculturalProduction || 0) + (portfolio.realEstate.farmland || 0);
    ...
    const businessTotal = commercialRE + cniUS + cniNonUS + agricultural + toDepository + otherSpecializedLoans + otherLeases;
    ...
    return {
      ...
      // Totals
      leasesTotal,
      grossTotal: consumerTotal + businessTotal
    };
  };
                              ...
                              {loanCategories.creditCards > 0 && (
                                <TableRow>
                                  <TableCell sx={{ pl: 2, fontSize: '0.8rem', py: 0.5 }}>
                                    Credit Cards
                                    <Tooltip
                                      title="Includes both consumer and commercial credit card receivables. The Call Report does not separate them."
                                      placement="right"
                                      arrow
                                    >
                                      <InfoOutlinedIcon sx={{ fontSize: 14, ml: 0.5, color: 'text.secondary', verticalAlign: 'middle', cursor: 'help' }} />
                                    </Tooltip>
                                  </TableCell>
                                  <TableCell align="right" sx={{ fontSize: '0.8rem', py: 0.5 }}>
