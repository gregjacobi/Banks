 ...
 */
const Sparkline = ({ data, width = 100, height = 24, color = '#d97757', showDot = true, periods = [] }) => {
 ...
 */
function TrendsTabCompact({ idrssd, availablePeriods, metadata = null }) {
  ...
  // Load TradingView widget for stock ticker
  useEffect(() => {
    console.log('[TrendsTabCompact] Stock widget check:', {
      hasMetadata: !!metadata,
      isPubliclyTraded: metadata?.ticker?.isPubliclyTraded,
      symbol: metadata?.ticker?.symbol,
      exchange: metadata?.ticker?.exchange,
      hasRef: !!widgetContainerRef.current
    });

    if (!metadata?.ticker?.isPubliclyTraded || !metadata?.ticker?.symbol) {
      console.log('[TrendsTabCompact] Stock widget NOT rendering - conditions not met');
      return;
    }
    ...
    const initWidget = () => {
      if (!widgetContainerRef.current) {
        console.log('[TrendsTabCompact] Widget container not ready yet, waiting...');
        setTimeout(initWidget, 100);
        return;
      }

      console.log('[TrendsTabCompact] Stock widget rendering for:', `${metadata.ticker.exchange}:${metadata.ticker.symbol}`);

      // Calculate date range based on available periods
      let dateRange = '12M'; // default
      if (availablePeriods && availablePeriods.length >= 2) {
        const sortedPeriods = [...availablePeriods].sort((a, b) => new Date(a) - new Date(b));
        const oldestDate = new Date(sortedPeriods[0]);
        const newestDate = new Date(sortedPeriods[sortedPeriods.length - 1]);
        const monthsDiff = (newestDate.getFullYear() - oldestDate.getFullYear()) * 12 +
                          (newestDate.getMonth() - oldestDate.getMonth());

        // TradingView supports: 1D, 5D, 1M, 3M, 6M, YTD, 12M, 60M, ALL
        if (monthsDiff <= 1) dateRange = '1M';
        else if (monthsDiff <= 3) dateRange = '3M';
        else if (monthsDiff <= 6) dateRange = '6M';
        else if (monthsDiff <= 12) dateRange = '12M';
        else dateRange = '60M'; // 5 years

        console.log(`[TrendsTabCompact] Calculated date range: ${dateRange} (${monthsDiff} months of data)`);
      }

      // Clear previous widget
      widgetContainerRef.current.innerHTML = '';

      // Create TradingView widget container and script
  ...
  const loanCategories = [
    ...
    // NOTE: "Unearned Income & Adjustments" is a contra-asset account (loan loss allowances, deferred fees)
    // It's not a loan category and should not be displayed in the portfolio breakdown
    ...
    <Box>
      ...
      <Paper sx={{ mb: 2, p: 2, bgcolor: '#fafafa' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5, borderBottom: '2px solid #333', pb: 0.5 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Sparkline data={loansData} color="#d97757" width={100} height={24} periods={sortedPeriods} />
          </Box>
        </Box>

        {/* Stacked Area Chart - Visual Overview */}
        <Box sx={{ mb: 3, mt: 2 }}>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart
              data={(() => {
                // Build data for stacked area chart - each period gets values for all categories
                // Each category already has time-series data in its `data` array
                return sortedPeriods.map((periodDate, idx) => {
                  const date = new Date(periodDate);
                  const year = date.getFullYear();
                  const quarter = Math.ceil((date.getMonth() + 1) / 3);
                  const dataPoint = { period: `${year} Q${quarter}` };

                  // Add each loan category value from its time-series data array
                  loanCategories.forEach(category => {
                    dataPoint[category.name] = (category.data[idx] || 0) / 1000; // Convert to millions
                  });
            ...
            >
              ...
              <YAxis
                tick={{ fontSize: 11 }}
                stroke="#666"
                label={{ value: '$M', angle: -90, position: 'insideLeft', style: { fontSize: 11 } }}
              />
              <Tooltip
                contentStyle={{ fontSize: 12, borderRadius: 4, border: '1px solid #ddd' }}
      ...
      <Paper sx={{ mb: 2, p: 2, bgcolor: '#fafafa' }}>
        ...
        <Grid container spacing={2}>
          ...
          {(() => {
            const shouldShow = metadata?.ticker?.isPubliclyTraded && metadata?.ticker?.symbol;
            console.log('[TrendsTabCompact] Stock widget JSX render check:', {
              shouldShow,
              metadata: metadata,
            ...
            return shouldShow && (
              <Grid item xs={12}>
                <Paper sx={{ p: 2, bgcolor: '#fff', border: '1px solid #e0e0e0', display: 'flex', flexDirection: 'column', height: 300 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
                    <Typography sx={{ fontSize: '0.75rem', color: 'text.secondary' }}>
                      Stock Performance
                    </Typography>
                    <Box sx={{ px: 1.5, py: 0.5, backgroundColor: '#d97757', color: 'white', borderRadius: 1, fontWeight: 600, fontSize: '0.75rem' }}>
                      {metadata.ticker.exchange || 'NYSE'}: {metadata.ticker.symbol}
                    </Box>
                  </Box>
                  <Box ref={widgetContainerRef} sx={{ flex: 1, minHeight: 0 }} />
                  <Typography sx={{ fontSize: '0.65rem', color: 'text.secondary', mt: 1, textAlign: 'center' }}>
                    12-month price chart • Source: TradingView
                  </Typography>
                </Paper>
              </Grid>
            );
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={incomeExpenseChartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
              ...
              <Legend
                ...
                iconSize={10}
              />
              <Bar dataKey="netInterestIncome" stackId="income" fill="#d97757" name="Net Interest" />
