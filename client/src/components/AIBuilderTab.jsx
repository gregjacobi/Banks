              ...
              {reportInProgress && (
                <>
                  ...
                  {currentPhase === 'agent_research' && agentMilestones.length > 0 && (
                    <Paper 
                      ref={milestonesRef}
                      sx={{ p: 2, mb: 2, bgcolor: '#f8f9ff', border: '1px solid #7c4dff', maxHeight: 200, overflow: 'auto' }}
                    >
                      <Typography variant="subtitle2" sx={{ fontWeight: 600, mb: 1, color: '#7c4dff' }}>
                        Agent Activity Log
                      </Typography>
                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                        {agentMilestones.slice(-10).map((milestone, idx) => (
                          <Box key={idx} sx={{ display: 'flex', gap: 1 }}>
                            <Chip label="✓" size="small" sx={{ bgcolor: '#7c4dff', color: 'white', height: 20, minWidth: 20 }} />
