...
class ClaudeService {
   ...
   * Wrapped with retry logic for timeout handling
  ...
  async searchForSources(query, category, bank, refinementPrompt = null) {
    // Wrap in retry logic to handle timeouts during web searches
    return this._withRetry(async (attempt) => {
      try {
        console.log(`[ClaudeService] searchForSources called for ${bank.name}, category: ${category} (attempt ${attempt}/${this.maxRetries})`);
        console.log(`[ClaudeService] Has refinement prompt: ${!!refinementPrompt}`);
...
- Prefer recent sources (last 12 months) but don't exclude slightly older high-quality sources
...
- **PRIORITY ORDER (CRITICAL - HIGHEST TO LOWEST VALUE):**
  1. **INVESTOR DAY & CONFERENCE PRESENTATION PDFs** (ABSOLUTE HIGHEST PRIORITY)
     - ⭐ These contain the richest strategic content and business insights!
     - Look for: "Investor Day", "Conference", "Summit", "Forum", "Roadshow"
     - Direct PDF links ending in .pdf
  2. **QUARTERLY EARNINGS PRESENTATION PDFs** (HIGH PRIORITY) - Direct PDF links ending in .pdf
  3. **EARNINGS TRANSCRIPTS** (MEDIUM PRIORITY) - Full text transcripts
  4. **PRESENTATIONS LANDING PAGES** (LOWER PRIORITY) - Only if they link to PDFs

- **PDF-FIRST STRATEGY:**
  - **ALWAYS prioritize PDF files** - they contain the full presentations
  - **INVESTOR DAY and CONFERENCE presentations are GOLD** - most strategic value
  - Look for URLs ending in .pdf (investor day decks, conference presentations, quarterly earnings presentations, supplements)
  - If you find a presentations page, LOOK AT THE CONTENT to extract direct PDF download links
  - Return those PDF links as separate individual sources

- **Where to search:**
  - investor.bankname.com/presentations
  - ir.bankname.com/events
  - bankname.com/investor-relations/presentations
  - Look in "Events & Presentations", "Earnings & Events", "Investor Materials" sections
  - Look for "Investor Day", "Conference Presentations", "Annual Meetings"

- **CRITICAL:**
  - When you find a presentations page, don't just return the page URL
  - EXTRACT all the PDF links from that page and return each PDF as its own source
  - Conference/Investor Day PDFs are MORE valuable than quarterly earnings PDFs
  - PDF files are infinitely more valuable than HTML pages

- **EXCLUDE:** Marketing materials, press releases, summaries without full content` : ''}
...
Return ONLY a valid JSON array, no other text.`;
        ...
        console.log(`[ClaudeService] Calling Claude API with web_search tool...`);
        ...
        console.log(`[ClaudeService] Claude API responded with ${response.content.length} content blocks`);
        console.log(`[ClaudeService] Stop reason: ${response.stop_reason}`);

        // Handle multi-turn conversation for web_search tool
        ...
        // If Claude paused to use tools, continue the conversation
        if (response.stop_reason === 'pause_turn') {
          console.log(`[ClaudeService] Handling pause_turn - continuing conversation...`);
          ...
          conversationMessages.push({
            role: 'user',
            content: [{ type: 'text', text: 'Continue - provide the JSON array of sources based on the search results.' }]
          });

          console.log(`[ClaudeService] Sending continuation request...`);
          ...
          console.log(`[ClaudeService] Continuation response - ${finalResponse.content.length} blocks, stop_reason: ${finalResponse.stop_reason}`);
        }
        ...
        for (const block of finalResponse.content) {
          if (block.type === 'text') {
            ...
            console.log(`[ClaudeService] Tool use detected: ${block.name}`);
          }
        }

        console.log(`[ClaudeService] Extracted response text length: ${responseText.length} chars`);
        if (responseText.length > 0) {
          console.log(`[ClaudeService] Response preview: ${responseText.substring(0, 200)}...`);
        }
        ...
        try {
          ...
          if (jsonMatch) {
            console.log(`[ClaudeService] Found JSON array in response`);
            const sources = JSON.parse(jsonMatch[0]);
            if (Array.isArray(sources)) {
              console.log(`[ClaudeService] Successfully parsed ${sources.length} sources`);
              return sources;
            } else {
              console.log(`[ClaudeService] Parsed value is not an array`);
              return [];
            }
          }
          console.log(`[ClaudeService] No JSON array found in response`);
          return [];
        } catch (parseError) {
          console.error('[ClaudeService] Error parsing sources JSON:', parseError);
          console.error('[ClaudeService] Response text:', responseText);
          return [];
        }

      } catch (error) {
        console.error('[ClaudeService] Error searching for sources (will retry if retryable):', error);
        throw error; // Let _withRetry handle retries
      }
    }); // End of _withRetry wrapper
  }
