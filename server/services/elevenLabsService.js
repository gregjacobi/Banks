...
class ElevenLabsService {
  constructor() {
    ...
    this.client = new ElevenLabsClient({
      apiKey: apiKey,
      timeout: 600000  // 10 minute timeout for long podcast generation
    });
  ...
  async generatePodcastAudio(segments, progressCallback = null) {
    try {
      ...
      if (!segments || segments.length === 0) {
        throw new Error('Cannot generate audio: No dialogue segments provided');
      }
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        // Generate audio for this chunk using text-to-dialogue with timeout protection
        const timeout = new Promise((_, reject) =>
          setTimeout(() => reject(new Error(`Text-to-dialogue timeout (5 minutes) for chunk ${i + 1}`)), 300000)
        );

        const audioStreamPromise = this.client.textToDialogue.convert({
