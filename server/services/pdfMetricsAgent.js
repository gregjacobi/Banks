    ...
    try {
      ...
      for (const pdf of pdfsToAnalyze) {
        try {
          ...
          if (pageCount > 100) {
            console.warn(`⚠️  Skipping PDF ${pdf.originalFilename}: ${pageCount} pages exceeds Claude's 100-page limit`);
          ...
          console.log(`Attached PDF: ${pdf.originalFilename} (${pageCount} pages, ${Math.round(pdf.fileSize / 1024)}KB)`);
        } catch (error) {
          console.error(`Error reading PDF ${pdf.originalFilename}:`, error.message);
        }
      }
