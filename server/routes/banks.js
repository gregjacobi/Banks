...
router.get('/:idrssd/peer-banks', async (req, res) => {
  try {
    ...
    const statements = await FinancialStatement.find({
      ...
      .select('idrssd ratios balanceSheet.assets')
      .lean();
    ...
    const banksData = statements.map(stmt => {
      ...
      const netLoans = stmt.balanceSheet?.assets?.earningAssets?.loansAndLeases?.net || 0;

      // Calculate loan portfolio percentages
      let loanMix = null;
      if (portfolio && netLoans > 0) {
        // Consumer loans
        ...
        const businessTotal = commercialRE + cniUS + cniNonUS;

        loanMix = {
          consumerPct: (consumerTotal / netLoans) * 100,
          residentialPct: (residential / netLoans) * 100,
          creditCardsPct: (creditCards / netLoans) * 100,
          autoPct: (auto / netLoans) * 100,
          otherConsumerPct: (otherConsumer / netLoans) * 100,
          businessPct: (businessTotal / netLoans) * 100,
          commercialREPct: (commercialRE / netLoans) * 100,
          cniPct: ((cniUS + cniNonUS) / netLoans) * 100
        };
      }
